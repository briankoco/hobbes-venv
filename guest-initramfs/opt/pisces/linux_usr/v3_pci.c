...
int main(int argc, char ** argv) {
	...
	if (pisces_add_pci_device(
		get_pisces_id_from_path(enclave_path),
		name, bus, dev, fn, true) != 0)
	{
	    printf("Error: Could not assign PCI device to enclave %s\n",
		enclave_path);
	    return -1;
	}
	...
	if (pisces_remove_pci_device(
		get_pisces_id_from_path(enclave_path),
		name, bus, dev, fn, true) != 0)
	{
	    printf("Error: Could not remove PCI device from enclave %s\n",
		enclave_path);
	    return -1;
	}
