	      ...
	      int       num_blocks)
{
	...
	if (base_addr == PISCES_ANY_MEMBLOCK) {
	    ...
	    int i = 0; 
	    int block_id = pet_addr_to_block_id(base_addr);

	    /* Check and error out if reserved */
	    for (i = 0; i < num_blocks; i++) {
	
		pet_mem_state_t block_status = pet_block_status(block_id + i);
	 
		if ((block_status == PET_BLOCK_RSVD) || 
		    (block_status == PET_BLOCK_INVALID)) {
		    ERROR("Invalid Block specified as boot memory\n");
		    goto mem_error;
		}

		if (block_status == PET_BLOCK_ONLINE) {
		    if (pet_offline_block(block_id) == -1) {
			ERROR("Error offlining block %d\n", block_id);
			goto mem_error;
		    }
		}
	    }

	    boot_env.base_addr  = base_addr;
	    boot_env.block_size = block_size * num_blocks;
	    boot_env.num_blocks = num_blocks;
	}
	...
	if (cpu_id == PISCES_ANY_CPU) {
	    ...
	    pet_cpu_state_t cpu_status = pet_cpu_status(cpu_id);

	    if ((cpu_status == PET_CPU_RSVD) || 
		(cpu_status == PET_CPU_INVALID)) {
		ERROR("Invalid CPU (%d) specified as the boot CPU\n", cpu_id);
		goto cpu_error;
	    }

	    if (cpu_status == PET_CPU_ONLINE) {
