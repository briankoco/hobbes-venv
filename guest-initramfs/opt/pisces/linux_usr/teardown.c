...
int
pisces_free(int pisces_id)
{
    return pet_ioctl_path(PISCES_PATH, PISCES_FREE_ENCLAVE, (void *)(uint64_t)pisces_id);
}
		...
		int online_mem) 
{
    ...
    snprintf(enclave_path, 128, DEV_PATH "%d", pisces_id);
	...
	snprintf(cpu_path, 128, PROC_PATH "enclave-%d/cpus", pisces_id);
	...
	snprintf(mem_path, 128, PROC_PATH "enclave-%d/memory", pisces_id);
	...
	snprintf(pci_path, 128, PROC_PATH "enclave-%d/pci", pisces_id);
	...
	if (pet_ioctl_fd(ctrl_fd, PISCES_CMD_SHUTDOWN, 0) != 0) {
	    printf("Error: Could not SHUTDOWN pisces\n");
	}
