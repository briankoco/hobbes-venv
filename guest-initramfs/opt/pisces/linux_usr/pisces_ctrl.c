		     ...
		     void     * argp)
{
    ...
    if (ctrl_fd < 0) {
	ERROR("Error opening enclave control channel (Pisces Enclave: %s)\n", enclave_path);
	free(enclave_path);
	return -1;
    }
		    ...
		    int numa_zone)
{
	...
	if (pisces_send_ctrl_cmd(pisces_id, PISCES_CMD_ADD_MEM, &mem_range) == -1) {
	    ERROR("Could not add memory to enclave (%d)\n", pisces_id);
	}
    }
	       ...
	       int numa_zone) 
{
    ...
    if (ret != num_blocks) {
	ERROR("Could not allocate %d memory blocks\n", num_blocks);
		...
		int numa_zone)
{
    ...
    if (ret != num_cpus) {
	ERROR("Could not allocate %d CPUs\n", num_cpus);
	...
	if (pisces_send_ctrl_cmd(pisces_id, PISCES_CMD_ADD_CPU, (void *)cpu_id) != 0) {
	    ERROR("Could not add CPU %lu to enclave %d\n", cpu_id, pisces_id);
		       ...
		       uint64_t cpu_id)
{
	...
	ERROR("CPU (%lu) is not in the OFFLINE state\n", cpu_id);
	return -1;
    }

    if (pisces_send_ctrl_cmd(pisces_id, PISCES_CMD_ADD_CPU, (void *)cpu_id) != 0) {
	ERROR("Could not add CPU %lu to enclave\n", cpu_id);
	return -1;
    }
	       ...
	       uint64_t cpu_id)
{
	...
	ERROR("Could not offline CPU %lu\n", cpu_id);
	return -1;
    }    

    if (pisces_add_offline_cpu(pisces_id, cpu_id) != 0) {
	ERROR("Could not add CPU %lu to enclave\n", cpu_id);
	pet_online_cpu(cpu_id);
	return -1;
    }
			  ...
			  uint64_t cpu_id)
{
	...
	ERROR("Error: Could not remove CPU %lu from enclave\n", cpu_id);
	return -1;
    }
...
pisces_remove_cpu(int      pisces_id,
		  uint64_t cpu_id)
{
    if (pisces_remove_offline_cpu(pisces_id, cpu_id) != 0) {
	ERROR("Error: Could not remove CPU %lu from enclave\n", cpu_id);
	return -1;
    }
	       ...
	       uint64_t    stack_size)
{
    ...
    if ((name     == NULL) || 
	(exe_path == NULL)) {
	ERROR("Missing name/exe parameter\n"); 
	return -1;
    }

    if (strlen(name) > 63) {
	ERROR("Name is too large (MAX=63)\n");
	return -1;
    }

    if (strlen(exe_path) > 255) {
	ERROR("exe path is too large (MAX=255)\n");
	return -1;
    }
	...
	if (strlen(exe_argv) > 255) {
	    ERROR("ARGV string is too large (MAX SIZE=255)\n");
	    return -1;
	}
	...
	if (strlen(envp) > 255) {
	    ERROR("ENVP string is too large (MAX=255)\n");
	    return -1;
	}
		      ...
		      bool     is_online)
{
	...
	printf("Error: Could not add device to enclave\n");
		         ...
		         bool     want_online)
{
	...
	printf("Error: Could not remove device from enclave\n");
	return -1;
    }
